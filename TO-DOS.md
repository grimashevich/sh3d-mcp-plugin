...
- [ ] render_photo overhead: параметр `focusOn` — фокус на конкретном объекте (`furniture:<id>`) или комнате (`room:<id>`), bbox объекта + padding вместо bbox всей сцены, для детальной проверки расстановки/поворота после изменений
- [ ] render_photo overhead: прозрачность стен — временно устанавливать wallsAlpha перед рендером, чтобы интерьер был виден со всех 4 углов (сейчас 2 из 4 ракурсов закрыты стенами)
- [ ] render_photo overhead: увеличить разрешение по умолчанию — текущие 800×600 дают мелкую мебель, рассмотреть 1200×900 или 1600×1200 (на современных CPU не критично)
- [ ] render_photo overhead: контраст на пустых сценах — без текстур всё бело-серое, рассмотреть автоматическое применение временного цвета пола или wireframe-режим для лучшей читаемости
- [ ] load_home — загрузка сцены из .sh3d файла (парный метод к save_home), позволяет переиспользовать сохранённые планировки вместо создания с нуля
