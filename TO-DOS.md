...
## Фаза 8 (v0.8) — UX-улучшения по результатам тестирования

Источник: `test_reports/test_log_2026-02-19.md` — отчёт агента-тестировщика, воспроизводившего квартиру по чертежу БТИ.

### Стабильные ID объектов [P0, High] — ПРОБЛЕМА #4

Текущие ID — это индексы массивов (`home.getWalls()`, `home.getFurniture()`, `home.getRooms()`). При удалении объекта все ID после него сдвигаются. Это ломает workflow агента: после `delete_wall id=3` бывший `id=4` становится `id=3`.

**Решение:** Перейти на стабильные ID. Варианты:
- a) SH3D `getId()` (внутренний String ID объекта) — если доступен и уникален
- b) Собственный маппинг UUID → объект в HomeAccessor
- c) Хранить маппинг int → object, не сдвигать при удалении (sparse array)

**Затронуты:** все хэндлеры, использующие ID — get_state, delete_*, modify_*, connect_walls, place_door_or_window, apply_texture, restore_checkpoint. Нужна миграция всех хэндлеров + тестов.

- [ ] Исследовать SH3D API на наличие стабильных ID у Wall/HomePieceOfFurniture/Room
- [ ] Выбрать подход и реализовать стабильные ID
- [ ] Мигрировать все хэндлеры на стабильные ID
- [ ] Обновить тесты

### Авто-чекпоинт перед крупными операциями [P1, Medium] — ПРОБЛЕМА #6

Агент забывает вызывать `checkpoint` перед batch/delete. При ошибке откат невозможен — приходится пересоздавать сцену с нуля.

- [ ] Параметр `auto_checkpoint: true` (default false) в `batch_commands` — автоматический снимок перед выполнением батча
- [ ] Авто-чекпоинт перед `clear_scene` (всегда, без параметра)

### Авто-подгонка depth дверей/окон под толщину стены [P1, Medium] — ПРОБЛЕМА #7

При размещении окна (depth=34 см) в стену (thickness=40 см) окно полностью скрыто внутри стены. Сейчас нужен отдельный `modify_furniture` для исправления.

- [ ] В `place_door_or_window`: если depth объекта < thickness стены, автоматически увеличить depth до thickness

### place_furniture: параметр elevation [P2, Low] — ПРОБЛЕМА #8

Для настенных предметов (вытяжка, навесной шкаф, ТВ) нужно 2 вызова: place + modify elevation. Можно сократить до одного.

- [ ] Добавить опциональный параметр `elevation` в `place_furniture` и `PlaceFurnitureHandler`

### modify_wall: перемещение стены (координаты) [P2, Low] — ПРОБЛЕМА #5

`modify_wall` изменяет толщину, высоту, цвет, дугу, но НЕ координаты. Чтобы передвинуть стену, нужен delete + create (теряются соединения, двери/окна).

- [ ] Добавить опциональные параметры `xStart`, `yStart`, `xEnd`, `yEnd` в `modify_wall`

### Толерантность MCP-сессии [P2, Low] — ПРОБЛЕМА #1

При истечении/потере MCP-сессии клиент Claude Code шлёт старый Session ID и получает ошибку. Сервер работает корректно, но можно сделать его толерантнее.

- [ ] При запросе с невалидным Session ID — автоматически создать новую сессию (вместо ошибки 404) или увеличить TTL сессий
