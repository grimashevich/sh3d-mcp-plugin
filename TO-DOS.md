...
### Уведомление при занятом порте [P1, Low]

При неудачном `start()` (порт занят) пользователь не получает никакой обратной связи — меню просто остаётся "Start". Нужно показывать ошибку.

- [ ] Показать диалог/уведомление при `BindException` (порт занят)
- [ ] Включить номер порта в сообщение об ошибке

### Настройка порта через UI [P2, Low]

Сейчас порт задаётся только через `PluginConfig` (файл/system property). Было бы удобно менять порт из интерфейса SH3D.

- [ ] Диалог настроек плагина (порт, autoStart)
- [ ] Валидация порта (1024-65535)
- [ ] Перезапуск сервера при смене порта
