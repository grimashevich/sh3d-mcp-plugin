...
### render_photo — размер ответа переполняет контекст [P1, High]

- [ ] Исследовать: Claude Desktop (mcp-remote) выдаёт ошибку "размер результата инструмента не может превышать 1 МБ" при получении рендера с base64
- [ ] Контекстное окно Claude Desktop быстро заканчивается из-за тяжёлых ответов (base64-изображения)
- [ ] Возможные решения: принудительный filePath, уменьшение разрешения по умолчанию, сжатие, ограничение размера base64-ответа, возврат только метаданных + путь к файлу
